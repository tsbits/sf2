...
var Mondrian=function(t,i,h,e,o){this.mapWidth=t||10,this.mapHeight=i||10,this.tileWidth=h||25,this.tileHeight=e||25,this.map=o||[],this.hero={x:10,y:5,speed:.2,startX:1,startY:1,invicible:!1,bag:[]},this.enemies=[],this.items=[],this.doors=[],this.gameEvents={collisions:new signals.Signal},this.buildMap()};Mondrian.prototype.buildMap=function(t){if(0==this.map.length)for(var i=0;i<this.mapHeight;i++){for(var h=[],e=0;e<this.mapWidth;e++)t?h.push(Math.round(Math.random())):h.push(1);this.map.push(h)}this.makeBorder()},Mondrian.prototype.makeBorder=function(){for(var t=0;t<this.mapHeight;t++)for(var i=0;i<this.mapWidth;i++)0==t||t==this.map.length-1?this.map[t][i]=0:0!=i&&i!=this.map[t].length-1||(this.map[t][i]=0)},Mondrian.prototype.makeWall=function(t,i,h,e){if(e)for(var o=0;h>o;o++)this.map[i+o][t]=0;else for(var o=0;h>o;o++)this.map[i][t+o]=0},Mondrian.prototype.render=function(){this.moveEnemies();for(var t=0;t<this.items.length;t++){var i=this.items[t];Math.round(this.hero.x)==i.x&&Math.round(this.hero.y)==i.y&&(this.pickUpItem(t),this.gameEvents.collisions.dispatch(i))}for(var t=0;t<this.enemies.length;t++){var h=this.enemies[t];Math.round(this.hero.x)==Math.round(h.x)&&Math.round(this.hero.y)==Math.round(h.y)&&this.gameEvents.collisions.dispatch(h)}for(var t=0;t<this.doors.length;t++){var e=this.doors[t];Math.round(this.hero.x)==e.x&&Math.round(this.hero.y)==e.y&&this.gameEvents.collisions.dispatch(e)}},Mondrian.prototype.moveHero=function(t,i,h){var e=1;"walk"==h&&(e=.25),0>t&&0!=this.map[Math.round(this.hero.y)][Math.ceil(this.hero.x)-1]&&(this.hero.x+=t*this.hero.speed*e),t>0&&0!=this.map[Math.round(this.hero.y)][Math.floor(this.hero.x)+1]&&(this.hero.x+=t*this.hero.speed*e),0>i&&0!=this.map[Math.ceil(this.hero.y)-1][Math.round(this.hero.x)]&&(this.hero.y+=i*this.hero.speed*e),i>0&&0!=this.map[Math.floor(this.hero.y)+1][Math.round(this.hero.x)]&&(this.hero.y+=i*this.hero.speed*e)},Mondrian.prototype.moveEnemies=function(){for(var t=0;t<this.enemies.length;t++){var i=this.enemies[t];if(0==i.dir&&0!=this.map[Math.round(i.y)][Math.ceil(i.x)-1])i.x-=i.speed;else if(2==i.dir&&0!=this.map[Math.round(i.y)][Math.floor(i.x)+1])i.x+=i.speed;else if(1==i.dir&&0!=this.map[Math.ceil(i.y)-1][Math.round(i.x)])i.y-=i.speed;else{if(3!=i.dir||0==this.map[Math.floor(i.y)+1][Math.round(i.x)])return void(i.dir=Math.round(3*Math.random()));i.y+=i.speed}Math.random()>.99&&(i.dir=Math.round(3*Math.random()))}},Mondrian.prototype.setHeroStart=function(t,i){this.hero.x=t,this.hero.startX=t,this.hero.y=i,this.hero.startY=i},Mondrian.prototype.setHeroSize=function(t,i){this.hero.width=t,this.hero.height=i},Mondrian.prototype.resetHeroPos=function(){this.hero.x=this.hero.startX,this.hero.y=this.hero.startY},Mondrian.prototype.addEnemy=function(t,i,h){this.enemies.push({x:t,y:i,speed:h,dir:Math.round(3*Math.random()),type:"enemy"})},Mondrian.prototype.addItem=function(t,i,h){this.items.push({x:t,y:i,name:h,type:"item",id:this.items.length})},Mondrian.prototype.addDoor=function(t,i,h){this.doors.push({x:t,y:i,targetLevel:h,type:"door"})},Mondrian.prototype.pickUpItem=function(t){this.hero.bag.push(this.items.splice(t,1)[0])},Mondrian.prototype.isTileWalkable=function(t,i){var h=!1;return 0!=this.map[i][t]&&(h=!0),h};
var Montana=function(){};Montana.randomColor=function(){for(var n="0123456789ABCDEF".split(""),a="#",t=0;6>t;t++)a+=n[Math.round(15*Math.random())];return a},Montana.randomColorFromScheme=function(n){var a=Montana.scheme[n][Math.round(Math.random()*(Montana.scheme[n].length-1))];return a},Montana.changeColorLuminance=function(n,a){n=String(n).replace(/[^0-9a-f]/gi,""),n.length<6&&(n=n[0]+n[0]+n[1]+n[1]+n[2]+n[2]),a=a||0;var t,e,o="#";for(e=0;3>e;e++)t=parseInt(n.substr(2*e,2),16),t=Math.round(Math.min(Math.max(0,t+t*a),255)).toString(16),o+=("00"+t).substr(t.length);return o},Montana.hexToRgb=function(n){var a=/^#?([a-f\d])([a-f\d])([a-f\d])$/i;n=n.replace(a,function(n,a,t,e){return a+a+t+t+e+e});var t=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(n);return t?{r:parseInt(t[1],16),g:parseInt(t[2],16),b:parseInt(t[3],16)}:null},Montana.componentToHex=function(n){var a=n.toString(16);return 1==a.length?"0"+a:a},Montana.rgbToHex=function(n,a,t){return"#"+this.componentToHex(n)+this.componentToHex(a)+this.componentToHex(t)},Montana.scheme={},Montana.scheme.beach=["#FFD946","#F9C74B","#2996C1","#268DA5","#FFFFFF"],Montana.scheme.bert=["#F0EDE6","#95BFB8","#63A497","#332936","#DCD5C1"],Montana.scheme.breizh=["#275D8C","#0578A6","#15A0BF","#BFB1A8","#F2D5C4"],Montana.scheme.cake=["#BF3B58","#5B3349","#F4C284","#F5A286","#F06363"],Montana.scheme.dawn=["#A49294","#282240","#2E2759","#323073","#BF5F56"],Montana.scheme.irland=["#005572","#006573","#008B8D","#81BEAA","#F4D4AD"],Montana.scheme.mint=["#005B4F","#157362","#61A486","#92D7B2","#C3F4D0"],Montana.scheme.nebula=["#D99F00","#D92B4B","#040740","#010626","#0596A6","#0BD9C4"],Montana.scheme.patriot=["#BF3542","#CDC5BA","#EBE3D6","#1F2A3C","#2E2E2E"],Montana.scheme.volcano=["#D6D0D9","#252622","#F27B35","#F2622E","#F2522E"];
